<script lang="ts">
	...
	import { boardData, defaultBoard, effectStore, pieceStore } from '$lib/state/stateStore';
		...
		for (let q = boardMeta.radius; q >= boardMeta.radius * -1; q--) {
			...
			for (let r = boardMeta.radius; r >= boardMeta.radius * -1; r--) {
				...
				if (newHex.inRadius(boardMeta.radius)) {
					hexArray.push(new HexData(newHex.q, newHex.r, boardMeta.hexSize, picker.next()));
				}
			}
		...
		<svg class="svgs" overflow="visible" viewBox={viewBoxString}>
			...
			{#key effects}
				...
				{#each effects.previous as i}
					<HexEffect q={i.q} r={i.r} color={Effects.PREVIOUS}></HexEffect>
				{/each}
				...
				{#if effects.current}
					<HexEffect q={effects.current.q} r={effects.current.r} color={Effects.CURRENT}
					></HexEffect>
				{/if}
